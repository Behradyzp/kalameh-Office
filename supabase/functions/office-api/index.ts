...
const permissionKeys: Record<string, string[]> = {
  ...
  "افزودن پروژه": ["projects"],
  "ویرایش پروژه": ["projects"],
  "حذف پروژه": ["projects"],
  ...
  "ویرایش تسک": ["tasks"],
  "حذف تسک": ["tasks"],
  "تغییر وضعیت تسک": ["tasks"],
  مالی: ["transactions"],
  "مشاهده مالی": ["transactions"],
  "مدیریت مالی": ["transactions"],
  مشتریان: ["clients"],
  "مشاهده مشتریان": ["clients"],
  "افزودن مشتری": ["clients"],
  "ویرایش مشتری": ["clients"],
  "حذف مشتری": ["clients"],
  لیدها: ["leads"],
  "مشاهده لیدها": ["leads"],
  "افزودن لید": ["leads"],
  ...
  "حذف لید": ["leads"],
  قراردادها: ["contracts"],
  "مشاهده قراردادها": ["contracts"],
  "مدیریت قراردادها": ["contracts"],
  "اعضای تیم": [],
  پیام‌ها: ["chats"],
  "ارسال پیام": ["chats"],
  نامه‌ها: ["letters"],
  "مشاهده نامه‌ها": ["letters"],
  "ایجاد نامه": ["letters"],
  تقویم: ["events"],
  "مشاهده تقویم": ["events"],
  "مدیریت تقویم": ["events"],
...
const allowedKeys = (
  ...
  for (const permission of Array.isArray(member?.permissions)
    ? member.permissions
    : []) {
    for (const key of permissionKeys[String(permission)] || [])
      allowed.add(key);
  }
  return allowed;
};
...
const requiredMutationPermission = (
  ...
  const current = (Array.isArray(currentValue) ? currentValue : []) as Record<
    string,
    unknown
  >[];
  const next = (Array.isArray(nextValue) ? nextValue : []) as Record<
    string,
    unknown
  >[];
  ...
  if (next.some((item) => !currentById.has(String(item.id))))
    return labels[key][0];
  if (current.some((item) => !nextById.has(String(item.id))))
    return labels[key][2];
  for (const item of next) {
    const before = currentById.get(String(item.id));
    if (before && JSON.stringify(before) !== JSON.stringify(item)) {
      if (key === "tasks") {
        const withoutStatus = (value: Record<string, unknown>) => {
          const copy = { ...value };
          delete copy.status;
        ...
        if (
          JSON.stringify(withoutStatus(before)) ===
          JSON.stringify(withoutStatus(item))
        )
          return "تغییر وضعیت تسک";
      }
      return labels[key][1];
    }
  }
...
const filterWorkspace = (
  ...
  const visibleProjects = projects.filter((project) => {
    ...
    return (
      !project.ownerId || project.ownerId === memberId || ids.includes(memberId)
    );
  });
  const projectNames = new Set(
    visibleProjects.map((project) => String(project.title || "")),
  );
  const tasks = (
  ...
  for (const [key, value] of Object.entries(result)) {
    if (key === "members") {
      result[key] = members.map((candidate) =>
        String(candidate.email || "").toLowerCase() === email.toLowerCase()
          ? candidate
          : { ...candidate, permissions: [] },
      );
    } else if (key === "projects")
      result[key] = allowed.has(key) ? visibleProjects : [];
...
const syncAuthMembers = async (
  admin: ReturnType<typeof createClient>,
  data: Record<string, unknown>,
) => {
  const { data: profiles, error } = await admin
    .from("profiles")
    .select("email,name,role,active,last_seen_at")
    .order("created_at");
  if (error) throw error;
  const existing = (Array.isArray(data.members) ? data.members : []) as Record<
    string,
    unknown
  >[];
  let nextId =
    existing.reduce((max, member) => Math.max(max, Number(member.id) || 0), 0) +
    1;
  ...
  for (const profile of profiles || []) {
    ...
    const member = {
      ...
      email: profile.email,
      role:
        profile.role === "admin" ? "مدیر کل" : String(base.role || "عضو تیم"),
      status: profile.active ? "فعال" : "غیرفعال",
...
const hydrateSignedUrls = async (
  ...
  const { data } = await admin.storage
    .from("office-files")
    .createSignedUrls(ordered, 3600);
  const urls = new Map(
    (data || []).map((item, index) => [ordered[index], item.signedUrl]),
  );
  const hydrate = (item: unknown): unknown => {
    ...
    if (typeof object.storagePath === "string")
      next.url = urls.get(object.storagePath) || "";
    return next;
  };
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (authError || !authData.user)
      return respond({ error: "ابتدا وارد حساب شوید." }, 401);
    ...
    const { data: profile } = await admin
      ...
      .single();
    if (!profile?.active)
      return respond({ error: "حساب کاربری غیرفعال است." }, 403);
    await admin
      .from("profiles")
    ...
    if (action === "workspace_get") {
      ...
      if (
        JSON.stringify(synced.members) !==
        JSON.stringify((row.data || {}).members)
      ) {
        ...
        if (updated) row.revision = updated.revision;
      }
      const visible = filterWorkspace(synced, profile.email, isAdmin);
      return respond({
        data: await hydrateSignedUrls(admin, visible),
        version,
    ...
    if (action === "workspace_save") {
      if (!body.data || typeof body.data !== "object")
        return respond({ error: "اطلاعات نامعتبر است." }, 400);
      const { data: row, error } = await admin
        .from("workspace_state")
        .select("data,updated_at,revision")
        .eq("id", "main")
      ...
      if (
        isAdmin &&
        requiredCollections.some((key) => !Array.isArray(submitted[key]))
      )
        return respond(
          { error: "ذخیره متوقف شد؛ اطلاعات یکی از بخش‌ها ناقص است." },
          422,
        );
      ...
      if (!isAdmin) {
        ...
        const permissions = new Set(
          (Array.isArray(member?.permissions) ? member.permissions : []).map(
            String,
          ),
        );
        for (const [key, value] of Object.entries(submitted)) {
          ...
          if (
            ...
            const required = requiredMutationPermission(
              key,
              current[key],
              value,
            );
            ...
            if (
              ...
              return respond(
                { error: `دسترسی «${required}» برای این حساب فعال نیست.` },
                403,
              );
          ...
          if (memberId === undefined)
            return respond(
              { error: "پروفایل عضو در فضای کاری پیدا نشد." },
              403,
            );
          if (key === "personalTasks") {
            const existingTasks = (
              Array.isArray(current.personalTasks) ? current.personalTasks : []
            ) as Record<string, unknown>[];
            const others = existingTasks.filter(
              (task) => task.ownerId !== memberId,
            );
            const own = (
              (Array.isArray(value) ? value : []) as Record<string, unknown>[]
            ).map((task) => ({
              ...(stripSignedUrls(task) as Record<string, unknown>),
              ownerId: memberId,
            }));
          ...
          if (key === "attendance") {
            const existingAttendance = (
              Array.isArray(current.attendance) ? current.attendance : []
            ) as Record<string, unknown>[];
            const others = existingAttendance.filter(
          ...
          const existing = (
            Array.isArray(current.leaves) ? current.leaves : []
          ) as Record<string, unknown>[];
          const incoming = (Array.isArray(value) ? value : []) as Record<
            string,
            unknown
          >[];
          const others = existing.filter(
            (leave) => leave.memberId !== memberId,
          );
          const ownExisting = new Map(
            existing
              .filter((leave) => leave.memberId === memberId)
              .map((leave) => [String(leave.id), leave]),
          );
          ...
          const own = incoming
            .filter(
              (leave) =>
                ...
                !protectedIds.has(String(leave.id)),
            )
            .map((leave) => {
      ...
      const { error: backupSaveError } = await admin
        .from("workspace_backups")
        .insert({ workspace_id: "main", data: current, saved_by: profile.id });
      if (backupSaveError)
        return respond(
          {
            error: "بکاپ ایمن ساخته نشد؛ برای جلوگیری از ریسک، ذخیره متوقف شد.",
          },
          503,
        );
      ...
      if (!saved)
        return respond(
          {
            error: "اطلاعات هم‌زمان توسط کاربر دیگری تغییر کرد.",
            revision: row.revision,
          },
          409,
        );
      await admin
        .from("audit_logs")
        .insert({ user_id: profile.id, action: "workspace_updated" });
      return respond({
        ok: true,
    ...
    if (action === "workspace_recover") {
      if (!isAdmin)
        return respond(
          { error: "فقط مدیر کل می‌تواند اطلاعات را بازیابی کند." },
          403,
        );
      ...
      const candidate = (backups || []).find(
        (backup) =>
          JSON.stringify(backup.data || {}) !== JSON.stringify(current),
      );
      if (!candidate)
        return respond(
          { error: "نسخه قبلی متفاوتی برای بازیابی پیدا نشد." },
          404,
        );
      const { error: recoveryBackupError } = await admin
        .from("workspace_backups")
        .insert({ workspace_id: "main", data: current, saved_by: profile.id });
      if (recoveryBackupError)
        return respond(
          { error: "بکاپ وضعیت فعلی ساخته نشد؛ بازیابی متوقف شد." },
          503,
        );
      ...
      return respond({
        data: await hydrateSignedUrls(admin, restored.data),
    ...
    if (action === "file_upload") {
      ...
      if (bytes.byteLength > 20 * 1024 * 1024)
        return respond({ error: "حداکثر حجم فایل ۲۰ مگابایت است." }, 413);
    ...
    if (action === "user_create") {
      if (!isAdmin)
        return respond({ error: "فقط مدیر کل به این بخش دسترسی دارد." }, 403);
      ...
      if (!email.includes("@") || password.length < 8 || !name)
        return respond(
          { error: "نام، ایمیل معتبر و رمز حداقل ۸ کاراکتری الزامی است." },
          422,
        );
      ...
      if (workspaceRow?.data) {
        ...
        await admin
          ...
          .eq("id", "main")
          .eq("revision", workspaceRow.revision);
      }
      await admin
        .from("audit_logs")
        .insert({
          user_id: profile.id,
          action: "user_created",
          metadata: { created_user_id: data.user.id, email },
        });
    ...
    if (action === "user_delete") {
      ...
      if (email === profile.email.toLowerCase())
        return respond({ error: "حساب مدیر فعلی قابل حذف نیست." }, 422);
      ...
      if (target.role === "admin")
        return respond({ error: "حذف مدیر دیگر از این بخش مجاز نیست." }, 422);
    ...
    if (action === "user_update") {
      ...
      if (!currentEmail || !email.includes("@") || !name)
        return respond({ error: "نام و ایمیل معتبر الزامی است." }, 422);
      if (password && password.length < 8)
        return respond({ error: "رمز جدید باید حداقل ۸ کاراکتر باشد." }, 422);
      const { data: target } = await admin
        .from("profiles")
        .select("id,role")
      ...
      await admin
        ...
        .insert({
          user_id: profile.id,
          action: "user_updated",
          metadata: { updated_user_id: target.id, email },
        });
    ...
    console.error(error);
    return respond(
      { error: error instanceof Error ? error.message : "خطای سرویس" },
      500,
    );
  }
});
