...
const userShape = (user: {
  ...
  email: user.email || "",
  name: String(
    user.user_metadata?.name || user.email?.split("@")[0] || "کاربر",
  ),
  role: String(user.user_metadata?.role || "member"),
});

async function invokeOfficeApi(body: Record<string, unknown>) {
  const { data, error } = await supabase.functions.invoke("office-api", {
    body,
  });
  if (error) {
...
async function uploadFile(body: FormData) {
  ...
  if (!(file instanceof File))
    return errorResponse("فایل انتخاب نشده است.", 400);
  const { data: sessionData } = await supabase.auth.getSession();
  const user = sessionData.session?.user;
  if (!user) return errorResponse("ابتدا وارد حساب شوید.", 401);
  if (file.size > 20 * 1024 * 1024)
    return errorResponse("حداکثر حجم فایل ۲۰ مگابایت است.", 413);
  ...
  if (error) {
    ...
    const fallback = await invokeOfficeApi({
      action: "file_upload",
      fileName: file.name,
      mimeType: file.type,
      base64: btoa(binary),
    ...
    return jsonResponse(
      fallback.payload,
      fallback.status >= 400 ? fallback.status : 201,
    );
  }
...
export function installSupabaseApiBridge() {
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (apiIndex < 0 && !raw.startsWith("api/"))
      return nativeFetch(input, init);
    if (!supabaseConfigured)
      return errorResponse("اتصال Supabase تنظیم نشده است.", 503);
    const route =
      apiIndex >= 0
        ? url.pathname.slice(apiIndex + 4)
        : raw.slice(3).split("?")[0];
    ...
    try {
      if (route === "/session") {
        ...
        return jsonResponse({
          user: result.payload?.user || userShape(data.session.user),
        });
      }
      if (route === "/auth/login" && method === "POST") {
        ...
        if (error || !data.user)
          return errorResponse(
            error?.message || "ایمیل یا رمز عبور صحیح نیست.",
            401,
          );
        const result = await invokeOfficeApi({ action: "session" });
        if (result.status >= 400)
          return jsonResponse(result.payload, result.status);
        return jsonResponse({
          user: result.payload?.user || userShape(data.user),
        });
      }
      ...
      if (route === "/auth/users" && method === "POST") {
        ...
        return jsonResponse(
          result.payload,
          result.status >= 400 ? result.status : 201,
        );
      }
      ...
      return errorResponse(
        reason instanceof Error ? reason.message : "ارتباط با سرویس انجام نشد.",
        503,
      );
    }
  };
